    ...
    try:
        if packet.haslayer(IP) and packet[IP].dst == host_ip:
            ...
            data = {
                ...
                'packet_length_variance': np.var(flows[flow_id]['fwd_packet_lengths'] + flows[flow_id]['bwd_packet_lengths']),
                ...
                'active_mean': np.mean(flows[flow_id]['fwd_packet_lengths']) if flows[flow_id]['fwd_packet_lengths'] else 0,
                'active_std': np.std(flows[flow_id]['fwd_packet_lengths']) if flows[flow_id]['fwd_packet_lengths'] else 0,
